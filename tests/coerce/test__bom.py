"""
Unit tests for BOM-level coercion coordinator (_bom.coerce_bom).

Example Usage:
    # Preferred usage via project-root invocation:
    python -m unittest tests/coerce/test__bom.py

    # Discover and run all tests (including this module):
    python -m unittest discover -s tests

Dependencies:
    - Python >= 3.10
    - Standard Library: unittest, typing, unittest.mock
    - Internal: src.models.interfaces, src.coerce._bom, src.coerce._header, src.coerce._row

Notes:
    - Tests isolate `_bom.coerce_bom` behavior using mocks for `_header` and `_row` to avoid deep recursion through individual field coercers.
    - Verifies that CoerceLog entries are properly formatted and contextualized.
    - Intended to ensure orchestration correctness, not individual rule behavior.
...
from src.coerce import _bom as cb  # Direct internal import — acceptable in tests
# noinspection PyProtectedMember
...
class TestCoerceBom(unittest.TestCase):
    ...
    def test_invalid(self):
        ...
        # ACT
        out_bom, log = cb.coerce_bom(src_dirty)
    ...
    def test_raise(self):
        ...
        # ACT
        with patch("src.coerce._bom._coerce_row", side_effect=ValueError("mapping failed")):
            try:
                _ = cb.coerce_bom(src)
                result = ""
            except ValueError:
...
class TestCoerceHeader(unittest.TestCase):
    ...
    def test_valid(self):
        ...
        # ACT
        out_header = cb._coerce_header(log, header)
        # ASSERT
        ...
        with self.subTest("Log size", Out=len(log.snapshot()), Exp=0):
    ...
    def test_invalid(self):
        ...
        log = common.CoerceLog()
        # ACT
        out_header = cb._coerce_header(log, dirty.boards[0].header)
        # ASSERT (spot-check)
        ...
        with self.subTest("Log size", Out=len(log.snapshot()), Exp=0):
            self.assertGreater(len(log.snapshot()), 0)
    ...
    def test_raises(self):
        ...
        # ACT
        with patch("src.coerce._bom.mdl.Header", side_effect=TypeError("bad mapping")):
            try:
                _ = cb._coerce_header(log, header)
                result = ""
            except ValueError:
                result = ValueError.__name__
...
class TestCoerceRow(unittest.TestCase):
    ...
    def test_valid(self):
        ...
        log = common.CoerceLog()
        # ACT
        out_row = cb._coerce_row(log, row)
        # ASSERT
    ...
    def test_invalid(self):
        ...
        # ACT
        out_row = cb._coerce_row(log, dirty)
    ...
    def test_raises(self):
        ...
        expected = ValueError.__name__
        # ACT
        with patch("src.coerce._bom.mdl.Row", side_effect=TypeError("bad mapping")):
            try:
                _ = cb._coerce_row(log, row)
                result = ""
            except ValueError:
                result = ValueError.__name__
